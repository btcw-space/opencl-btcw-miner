...
int main(int argc, char* argv[]) {
    ...
    if (user_work_size > 0) {
        ...
        // Auto-tune: saturate all CUs with ~8x oversubscription to hide latency
        GLOBAL_WORK_SIZE = (size_t)sel.compute_units * LOCAL_WORK_SIZE * 8;
        // Clamp to reasonable bounds
        if (GLOBAL_WORK_SIZE < 32768)   GLOBAL_WORK_SIZE = 32768;    // minimum floor
        if (GLOBAL_WORK_SIZE > 2097152) GLOBAL_WORK_SIZE = 2097152;  // 2M cap
