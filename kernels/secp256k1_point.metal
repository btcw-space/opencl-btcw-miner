...
#include "secp256k1_field.metal"

// =============================================================================
// secp256k1 Metal - Point Operations
// =============================================================================
// Copyright (c) 2026 btcw.space <btcw.space@proton.me>
//
// Elliptic curve point operations on secp256k1: y^2 = x^3 + 7
...
struct JacobianPoint {
    ...
    FieldElement y;
    FieldElement z;
    uint32_t infinity;  // 1 if point at infinity
};
...
static inline void point_set_infinity(thread JacobianPoint* p) {
...
static inline int point_is_infinity(thread const JacobianPoint* p) {
    return p->infinity ||
           ((p->z.limbs[0] | p->z.limbs[1] | p->z.limbs[2] | p->z.limbs[3]) == 0);
}

static inline void point_from_affine(thread JacobianPoint* j, thread const AffinePoint* a) {
    j->x = a->x;
    j->y = a->y;
...
static inline void get_generator(thread AffinePoint* g) {
...
static inline void point_double_impl(thread JacobianPoint* r, thread const JacobianPoint* p) {
...
static inline void point_add_impl(thread JacobianPoint* r, thread const JacobianPoint* p, thread const JacobianPoint* q) {
...
static inline void point_add_mixed_impl(thread JacobianPoint* r, thread const JacobianPoint* p, thread const AffinePoint* q) {
    if (point_is_infinity(p)) {
...
static inline int scalar_is_zero(thread const Scalar* k) {
    return (k->limbs[0] | k->limbs[1] | k->limbs[2] | k->limbs[3]) == 0;
}

static inline int scalar_bit(thread const Scalar* k, int pos) {
    int limb = pos / 64;
    int bit = pos % 64;
    return (int)((k->limbs[limb] >> bit) & (uint64_t)1);
}

static inline void scalar_sub_u64(thread Scalar* a, uint64_t val, thread Scalar* r) {
    *r = *a;
    uint64_t old = r->limbs[0];
    r->limbs[0] -= val;
    if (r->limbs[0] > old) { // borrow
        for (int i = 1; i < 4; i++) {
            r->limbs[i] -= 1;
            if (r->limbs[i] != ~(uint64_t)0) break; // no further borrow
        }
    }
}

static inline void scalar_add_u64(thread Scalar* a, uint64_t val, thread Scalar* r) {
    *r = *a;
    uint64_t old = r->limbs[0];
    r->limbs[0] += val;
...
static inline int scalar_to_wnaf(thread const Scalar* k, thread int* wnaf) {
    Scalar temp = *k;
    int len = 0;
...
static inline void point_negate_y(thread JacobianPoint* p) {
...
static inline void scalar_mul_impl(thread JacobianPoint* r, thread const Scalar* k, thread const AffinePoint* p) {
...
static inline void scalar_mul_generator_impl(thread JacobianPoint* r, thread const Scalar* k) {
    AffinePoint G;
    get_generator(&G);
