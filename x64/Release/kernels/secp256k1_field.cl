// =============================================================================
// secp256k1 OpenCL - Field Arithmetic
// =============================================================================
...
inline void field_mul_impl(FieldElement* r, const FieldElement* a, const FieldElement* b) {
    ...
    // Row 1: a1 * b[0..3]
    // NOTE: All three-operand additions use two-step carry detection.
    // The pattern (t < A) + (t < C) for t = A+B+C is WRONG and can
    // miss or double-count carries. Two-step is provably correct.
    m = mul64_full(a1, b0);
    ...
    m = mul64_full(a1, b1);
    { ulong s1 = product[2] + m.x;
      ulong c1 = (s1 < product[2]) ? 1UL : 0UL;
      t = s1 + carry;
      ulong c2 = (t < s1) ? 1UL : 0UL;
      carry = m.y + c1 + c2; }
    product[2] = t;

    m = mul64_full(a1, b2);
    { ulong s1 = product[3] + m.x;
      ulong c1 = (s1 < product[3]) ? 1UL : 0UL;
      t = s1 + carry;
      ulong c2 = (t < s1) ? 1UL : 0UL;
      carry = m.y + c1 + c2; }
    product[3] = t;
    ...
    m = mul64_full(a1, b3);
    { ulong s1 = product[4] + m.x;
      ulong c1 = (s1 < product[4]) ? 1UL : 0UL;
      t = s1 + carry;
      ulong c2 = (t < s1) ? 1UL : 0UL;
      carry = m.y + c1 + c2; }
    product[4] = t;
    ...
    m = mul64_full(a2, b3);
    { ulong s1 = product[5] + m.x;
      ulong c1 = (s1 < product[5]) ? 1UL : 0UL;
      t = s1 + carry;
      ulong c2 = (t < s1) ? 1UL : 0UL;
      carry = m.y + c1 + c2; }
    product[5] = t;
    ...
    m = mul64_full(a3, b3);
    { ulong s1 = product[6] + m.x;
      ulong c1 = (s1 < product[6]) ? 1UL : 0UL;
      t = s1 + carry;
      ulong c2 = (t < s1) ? 1UL : 0UL;
      carry = m.y + c1 + c2; }
    product[6] = t;
...
inline void field_sqr_impl(FieldElement* r, const FieldElement* a) {
    ...
    // NOTE: Three-operand additions use two-step carry detection
    // to avoid the (t < A) single-check bug that misses carries.
    m = mul64_full(a0, a0);
    product[0] = m.x;
    t = product[1] + m.y;
    ...
    m = mul64_full(a1, a1);
    { ulong s1 = product[2] + m.x;
      ulong ca = (s1 < product[2]) ? 1UL : 0UL;
      t = s1 + carry;
      ulong cb = (t < s1) ? 1UL : 0UL;
      carry = ca + cb; }
    product[2] = t;
    { ulong s1 = product[3] + m.y;
      ulong ca = (s1 < product[3]) ? 1UL : 0UL;
      t = s1 + carry;
      ulong cb = (t < s1) ? 1UL : 0UL;
      carry = ca + cb; }
    product[3] = t;
    ...
    m = mul64_full(a2, a2);
    { ulong s1 = product[4] + m.x;
      ulong ca = (s1 < product[4]) ? 1UL : 0UL;
      t = s1 + carry;
      ulong cb = (t < s1) ? 1UL : 0UL;
      carry = ca + cb; }
    product[4] = t;
    { ulong s1 = product[5] + m.y;
      ulong ca = (s1 < product[5]) ? 1UL : 0UL;
      t = s1 + carry;
      ulong cb = (t < s1) ? 1UL : 0UL;
      carry = ca + cb; }
    product[5] = t;
